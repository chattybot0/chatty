...
var updated = "";
client.login("NzQ5MTMzNDgzNzI0NjM2MjQw.X0ni7A.UzGgqvSKcS0a9Bll0Uh21i5Ri5M");
...
client.on("message", msg => {
  if (msg.author.bot) {
    ...
    msg.reply(
      "This is not a valid use,you should use it like this:\n!create~TOKEN~NAME~CLIENTID\nOr you can use !truthdare~TOKEN~NAME~CLIENTID to create a truth or date bot.**Use !self instead of create to make a bot maker for yourself!!!**"
    );
  ...
  } else if (msg.content.split("~")[0] == "!self") {
    var splat = msg.content.split("~");
    msg.channel.send("please wait until we create your bot...");
    fs.readFile("creator.txt", "utf8", function(err, contents) {
      updated = contents.replace("tokentoken", splat[1]);
      fs.writeFile("bots/" + splat[2] + ".js", updated, function(err) {
        fs.readFile("list.txt", "utf8", function(err, contentz) {
          fs.writeFile("list.txt", contentz + "''" + splat[2] + ".js", function(
            err
